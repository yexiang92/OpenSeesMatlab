...
- Add MATLAB callback-backed uniaxial materials through ``ops.callbackUniaxialMaterial(...)``. User-defined callbacks can provide stress, tangent and history-dependent state while supporting OpenSees trial, commit and revert semantics for failed-step recovery.
- Add MATLAB numerical substructure analysis through ``ops.callbackSubstructure``. A MATLAB callback can provide the resisting force, current tangent, mass and damping matrices for an OpenSees Element while OpenSees manages the global analysis. Support arbitrary substructure interfaces defined by ``[nodeTag, DOF]`` pairs, including a single condensed interface node or multiple physical end nodes.
