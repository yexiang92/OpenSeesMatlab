%% *Trust-region Methods for a Strongly Nonlinear Truss*
% A coarse load increment drives the yielding corotational truss through a difficult 
% equilibrium region. The short example isolates the trust-region update from 
% the underlying finite-element model.
% 
% This example compares the standard Newton algorithm with two TrustRegion subproblem 
% methods. The three-bar truss uses a corotational formulation and a yielding 
% steel material, so both geometric and material nonlinearities are present.
% 
% Only four large load steps are used. Newton cannot pass the third step, while 
% the trust-region methods complete the same loading path.
...
% Reading the result
% The Newton curve stops at the last converged load level. Both trust-region 
% variants reach the target load; the shared converged points confirm that step 
% control changes the solution process, not the equilibrium path.